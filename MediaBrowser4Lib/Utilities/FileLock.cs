...
namespace MediaBrowser4.Utilities
{
    ...
    {
        public static object UnsafeGetHandkesLockedBy(Process process, string filename)
        {
            try
            {
                ...
                foreach (var handle in handles)
                {
                    try
                    {
                        ...
                        if (file != null && file.Equals(filename, StringComparison.InvariantCultureIgnoreCase))
                        {
                            return handle;
                        }
                    }
